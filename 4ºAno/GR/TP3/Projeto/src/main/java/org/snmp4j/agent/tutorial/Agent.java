...
public class Agent implements VariableProvider {
    ...
    static List listaEventos = new ArrayList<Eventos>();
    ...
    public static void leFicheiro(){
        ...
        try {
            ...
            while (myReader.hasNextLine()) {
                ...
                Eventos e = new Eventos(tokens[0], LocalDateTime.parse(tokens[1],formatter),tokens[2],tokens[3],tokens[4],tokens[5]);
                listaEventos.add(e);
            }
            myReader.close();
    ...
    public void updateMIB(){
        ...
        for(Object e : listaEventos) {

            Eventos e1 = (Eventos) e;

            if (local.isAfter(LocalDateTime.parse(e1.getValidade(), formatter))) {
                modules.getGrEventsMib().getGrMIBEventsEntry().removeRow(new OID(String.valueOf(counter)));
            } else {
                GrEventsMib.GrMIBEventsEntryRow currentRow = modules.getGrEventsMib().getGrMIBEventsEntry().createRow(new OID(String.valueOf(counter)));
                currentRow = e1.updateEntryRow(currentRow, counter);
                modules.getGrEventsMib().getGrMIBEventsEntry().addRow(currentRow);
                counter++;
            }
        }
    ...
    public void chekaMIB(){
        int counter=1;
        ReentrantLock lock = new ReentrantLock();
        lock.lock();
        for(Object e : listaEventos){
            GrEventsMib.GrMIBEventsEntryRow currentRow = modules.getGrEventsMib().getGrMIBEventsEntry().getModel().getRow(new OID(String.valueOf(counter)));
